...
class Radium_Contact_Form extends Radium_Contact_Form_Shortcode {
	...
	function process_submission() {
		...
		$message .= __( 'Contact Form URL:', 'radium' ) . " $url\n";
		...
		$message = Radium_Contact_Form_Plugin::strip_tags( $message );
		...
		foreach ( (array) $akismet_values as $av_key => $av_value ) {
			$akismet_values[$av_key] = Radium_Contact_Form_Plugin::strip_tags( $av_value );
		}

		foreach ( (array) $all_values as $all_key => $all_value ) {
			$all_values[$all_key] = Radium_Contact_Form_Plugin::strip_tags( $all_value );
		}

		foreach ( (array) $extra_values as $ev_key => $ev_value ) {
			$extra_values[$ev_key] = Radium_Contact_Form_Plugin::strip_tags( $ev_value );
		}
 ...
 * Validates input.
 */
class Radium_Contact_Form_Field extends Radium_Contact_Form_Shortcode {
	var $shortcode_name = 'contact-field';

	/**
	 * @var Radium_Contact_Form parent form
	 */
	var $form;
	 ...
	 * @param array $attributes An associative array of shortcode attributes.  @see shortcode_atts()
	 * @param null|string $content Null for selfclosing shortcodes.  The inner content otherwise.
	 * @param Radium_Contact_Form $form The parent form
	 */
	...
	function validate() {
		...
		case 'email' :
			...
			if ( !is_email( $field_value ) ) {
				$this->add_error( sprintf( __( '%s requires a valid email address', 'radium' ), $field_label ) );
			}
			break;
		default :
			// Just check for presence of any text
			if ( !strlen( trim( $field_value ) ) ) {
				$this->add_error( sprintf( __( '%s is required', 'radium' ), $field_label ) );
			}
		}
	}
	...
	function render() {
		...
		$field_value = Radium_Contact_Form_Plugin::strip_tags( $this->value );
		$field_label = Radium_Contact_Form_Plugin::strip_tags( $field_label );

		switch ( $field_type ) {
		case 'email' :
			$r .= "\n<div class='input-text-block'>\n";
			$r .= "\t\t<label for='" . esc_attr( $field_id ) . "' class='radium_form-field-label email" . ( $this->is_error() ? ' form-error' : '' ) . "'>" . esc_html( $field_label ) . ( $field_required ? '<span class="required">&nbsp;*</span>' : '' ) . "</label>\n";
			$r .= "\t\t<input type='email' name='" . esc_attr( $field_id ) . "' id='" . esc_attr( $field_id ) . "' value='" . esc_attr( $field_value ) . "' class='email' />\n";
			$r .= "\t</div>\n";
			break;
		case 'textarea' :
			$r .= "\n<div class='input-text-block'>\n";
			$r .= "\t\t<label for='contact-form-comment-" . esc_attr( $field_id ) . "' class='radium_form-field-label textarea" . ( $this->is_error() ? ' form-error' : '' ) . "'>" . esc_html( $field_label ) . ( $field_required ? '<span class="required">&nbsp;*</span>' : '' ) . "</label>\n";
			$r .= "\t\t<textarea name='" . esc_attr( $field_id ) . "' id='contact-form-comment-" . esc_attr( $field_id ) . "' rows='20'>" . esc_textarea( $field_value ) . "</textarea>\n";
			$r .= "\t</div>\n";
			break;
		case 'radio' :
			$r .= "\t<div class='input-text-block'><label class='radium_form-field-label" . ( $this->is_error() ? ' form-error' : '' ) . "'>" . esc_html( $field_label ) . ( $field_required ? '<span class="required">&nbsp;*</span>' : '' ) . "</label>\n";
			foreach ( $this->get_attribute( 'options' ) as $option ) {
				$option = Radium_Contact_Form_Plugin::strip_tags( $option );
		...
		case 'checkbox' :
			$r .= "\n<div class='input-text-block'>\n";
			$r .= "\t\t<label class='radium_form-field-label checkbox" . ( $this->is_error() ? ' form-error' : '' ) . "'>\n";
			$r .= "\t\t<input type='checkbox' name='" . esc_attr( $field_id ) . "' value='" . esc_attr__( 'Yes', 'radium' ) . "' class='checkbox' " . checked( (bool) $field_value, true, false ) . " /> \n";
			$r .= "\t\t" . esc_html( $field_label ) . ( $field_required ? '<span class="required">&nbsp;*</span>' : '' ) . "</label>\n";
			$r .= "\t\t<div class='clear-form'></div>\n";
			$r .= "\t</div>\n";
			break;
		case 'select' :
			$r .= "\n<div class='input-text-block'>\n";
			$r .= "\t\t<label for='" . esc_attr( $field_id ) . "' class='radium_form-field-label select" . ( $this->is_error() ? ' form-error' : '' ) . "'>" . esc_html( $field_label ) . ( $field_required ? '<span class="required">&nbsp;*</span>' : '' ) . "</label>\n";
		...
		default : // text field
			...
			$r .= "\n<div class='input-text-block'>\n";
			$r .= "\t\t<label for='" . esc_attr( $field_id ) . "' class='radium_form-field-label " . esc_attr( $field_type ) . ( $this->is_error() ? ' form-error' : '' ) . "'>" . esc_html( $field_label ) . ( $field_required ? '<span class="required">&nbsp;*</span>' : '' ) . "</label>\n";
...
add_action( 'init', array( 'Radium_Contact_Form_Plugin', 'init' ) );
